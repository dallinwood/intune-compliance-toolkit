...
Two different artifacts use two different naming/location conventions - don't mix them up:

- **Control specs** (the fact-only intermediate this schema's rules are authored from, for `independent`-mode content - see `SKILL.md`'s "Which workflow applies to you") live at `specs/<framework>/<product>/<version>/<control_id>.json` (e.g. `specs/cis/windows_11/5.0.0/18.9.31.2.json`). `<control_id>` there is the source framework's own numbering for the control the spec was produced from, not a toolkit-assigned identifier - specs don't have one of those, only rules do. This mirrors the `<framework>/<product>/<version>/` layout `baselines/` used before the rule library flattened (below): a spec genuinely is produced while processing one specific framework's content, so that hierarchy still means something here. See `specs/_control_spec.schema.json` for the spec's own field-by-field shape - this file documents rule JSON only.
- **Rules** live at `baselines/rules/rule_<id>_<short-title-slug>.json` (e.g. `baselines/rules/rule_12_ensure-example-setting-is-configured.json`) - `id` is the rule's own toolkit-assigned identifier (see "Top level" below), not any framework's numbering, since one rule can map to several frameworks at once. The title slug is a lowercased, hyphenated shortening of `title`, kept short enough to stay scannable; it exists for humans browsing a folder listing and carries no meaning the JSON itself doesn't already state - the filename is never authoritative, `id` inside the file is. The literal `rule_` prefix isn't decoration: the filename is also the source of every variable-name prefix in the rule's `check_command`s (see "Variable naming" below), and an identifier may not begin with a digit in either PowerShell or bash, which a bare `<id>_...` name would. JSON only, no YAML. Unlike `specs/`, `baselines/` is a single flat directory, not nested by framework/product/version - a rule can map to more than one framework and isn't "owned" by any single one, so all filtering and browsing is done via `framework_mappings`/`policy_classification`, never folder location.

**`baselines/rules/` is a project decision, not part of this schema, and it has changed once already** (it used to be nested by framework/product/version, the way `specs/` still is, before the rule library flattened to one directory). If you need existing rules as reference examples and `baselines/rules/` doesn't have any yet, search the repo for files matching the naming pattern above rather than assuming they don't exist somewhere else.
