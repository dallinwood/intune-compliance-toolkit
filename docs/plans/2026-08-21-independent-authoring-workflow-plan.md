...
- **2026-08-25** - Task 3 review fix: a coordinator review of Task 3 found that SKILL.md's "copy `framework_mapping_refs` into `framework_mappings`" instruction had nothing to copy `framework_product` from - Task 1's `specs/_control_spec.schema.json` `frameworkMappingRef` shape never had that field, while the rule schema's `frameworkMapping` shape requires it (nullable). Fixed by adding `framework_product` to the spec schema's `frameworkMappingRef` (mirroring `baselines/_rule.schema.json`'s `frameworkMapping` field-for-field), updating `tests/test_control_spec_schema.py`'s fixture plus two new tests (required; accepts `null`), and updating SKILL.md's "producing a spec" workflow to fill it. `pytest` - 76 passed (up from 74), 4 skipped. Commit `e8ec46b fix: add framework_product to the control-spec schema's frameworkMappingRef, closing a task 3 review finding`.
